...
// of the merge.
// extraRoles adds optional "Character 2"/"Cast type 2" (and " 3") column
// pairs for people who can play more than one role in the same show — only
// actors need this, so it defaults to 0 for crew/musicians/staff and their
// sheets are unaffected.
function peopleSpec({ filename, roleLabel, roleField, categoryLabel, categoryMap, sample, extraRoles = 0 }) {
  const roleColumnPairs = [[roleLabel, categoryLabel]];
  for (let i = 2; i <= extraRoles + 1; i++) {
    roleColumnPairs.push([`${roleLabel} ${i}`, `${categoryLabel} ${i}`]);
  }
  return {
    filename,
    columns: [C('Name', true), ...roleColumnPairs.flatMap(([r, c]) => [C(r), C(c)]), C('Phone'), C('Email')],
    sample,
    build: (r, ctx) => {
      const cmap = categoryMap(ctx) || {};
      const person = {
        id: uid('p'),
        name: r.get('Name'),
        phone: r.get('Phone') || '',
      ...
      if (!ctx.show) return person;
      roleColumnPairs.forEach(([roleCol, catCol]) => {
        const role = r.get(roleCol);
        if (!role) return;
        const catName = r.get(catCol) || '';
        const catKey = Object.keys(cmap).find(
          (k) => String((cmap[k] || {}).label || '').toLowerCase() === catName.toLowerCase()
        ...
        person.assignments.push({
          ...
          showId: ctx.show.id,
          [roleField]: role,
          [roleField === 'role' ? 'dept' : 'category']: catKey || Object.keys(cmap)[0],
        });
      });
      return person;
    },
